...
export default async function Home({ params }: { params:Promise<{locale:string}> }) {
  ...
  return <>
    <section className="hero-simple border-b border-black/20"><video className="hero-video" autoPlay muted loop playsInline preload="metadata" aria-hidden="true"><source src="/media/hero-color-web.mp4" type="video/mp4" /></video><div className="wrap hero-content flex flex-col items-center text-center"><h1 className="hero-title">{locale === "en" ? <><span>Do You Want to Increase</span><span>Your Conversion Rate?</span></> : <><span>¿Quieres aumentar tu tasa</span><span>de conversión?</span></>}</h1><p className="hero-copy mt-7 max-w-3xl">{c.hero.body}</p><TrackedLink className="button hero-button mt-9" href={`/${locale}/contact`}>{c.hero.cta}</TrackedLink></div></section>
