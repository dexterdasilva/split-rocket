...
export default async function Home({ params }: { params:Promise<{locale:string}> }) {
  ...
  return <>
    <section className="hero-simple border-b border-black/20"><div className="wrap flex flex-col items-center text-center"><h1 className="hero-title max-w-4xl">{c.hero.title}</h1><p className="hero-copy mt-7 max-w-3xl">{c.hero.body}</p><TrackedLink className="button hero-button mt-9" href={`/${locale}/contact`}>{c.hero.cta}</TrackedLink></div></section>
